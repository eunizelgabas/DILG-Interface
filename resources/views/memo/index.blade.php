<x-app-layout>
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center">
        <div class="md:mr-4"> <!-- Adjust margin based on your design -->
            <h1 class="font-semibold text-2xl mb-2 tracking-wider drop-shadow-[3px_3px_5px_rgba(91,91,91,0.58)]">Memo Circulars Archive</h1>
    ...
    <div class="flex-1 pr-4">
        <div class="relative md:w-1/3">
            <form action="{{ route('memo.index') }}" method="GET" class="mb-4" id="searchForm">
    ...
    <div class='flex items-center mt-3'>
        ...
        <div class="flex w-full items-center justify-between border-b pb-3">
          <div class="flex items-center space-x-3">
            ...
            <div class="text-ml font-bold text-slate-700">Reference No: <span class="font-light">{{$memo->issuance->reference_no}}</span></div>
          </div>
          <div class="flex items-center space-x-8">
            <div class="text-xs text-neutral-500">{{ \Carbon\Carbon::parse($memo->issuance->date)->format('F j, Y') }}</div>
            <div x-data="{ dropdownOpen: false }" class="relative">
                ...
                <div x-show="dropdownOpen" class="absolute right-0 mt-2 py-2 w-48 bg-white rounded-md shadow-xl z-20">
                    <a  href="{{ url('/memo_circulars/edit', $memo->id) }}" class="block px-4 py-2 text-sm capitalize text-gray-700 hover:bg-blue-500 hover:text-white">
                       Edit
                    </a>
        ...
        <div class="mt-4 mb-">
          ...
          <div class="flex-1 inline-flex items-center">
            ...
            <a href="{{ $memo->issuance->url_link }}" class="font-bold ml-1 hover:underline" target="_blank">
             <span class=" text-blue-500 font-light">{{ $memo->issuance->url_link }}</span>
            </a>
        </div>
        </div>
          ...
          <div class="flex items-center justify-between text-slate-500">
            <div class="flex space-x-4 md:space-x-8">
              <div class="flex cursor-pointer items-center transition hover:text-slate-600">
                ...
                <span class="ml-2"> {{$memo->issuance->keyword}}</span>
              </div>
    ...
    <div class="text-start" style="color:rgb(83, 82, 82); margin-top: 15px;">
        {{-- Showing {{ $memos->firstItem() }} to {{ $memos->lastItem() }} of {{ $memos->total() }} entries --}}
    </div>

    <div class="d-flex justify-content-end mt-2">
        {{ $memos->onEachSide(1)->links() }}
    </div>
    @else
    <div class="flex justify-center items-center">
        <h1>No Memo Circular available</h1>
    </div>
    @endif
