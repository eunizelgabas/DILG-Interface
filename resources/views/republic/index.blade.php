<x-app-layout>
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center">
        <div class="md:mr-4"> <!-- Adjust margin based on your design -->
            <h1 class="font-semibold text-2xl mb-2 tracking-wider drop-shadow-[3px_3px_5px_rgba(91,91,91,0.58)]">Republic Acts Archive</h1>
    ...
    <div class='flex items-center mt-3'>
        ...
        <div class="flex w-full items-center justify-between border-b pb-3">
          <div class="flex items-center space-x-3">
            ...
            <div class="text-ml font-bold text-slate-700">Reference No: <span class="font-light">{{$republic->issuance->reference_no}}</span></div>
          </div>
          <div class="flex items-center space-x-8">
            <div class="text-xs text-neutral-500">{{ \Carbon\Carbon::parse($republic->issuance->date)->format('F j, Y') }}</div>
            <div x-data="{ dropdownOpen: false }" class="relative">
                ...
                <div x-show="dropdownOpen" class="absolute right-0 mt-2 py-2 w-48 bg-white rounded-md shadow-xl z-20">
                    <a  href="{{ url('/republic_acts/edit', $republic->id) }}" class="block px-4 py-2 text-sm capitalize text-gray-700 hover:bg-blue-500 hover:text-white">
                       Edit
                    </a>

                    <a onclick="openDeleteModal({{ $republic->id }})" class="block px-4 py-2 text-sm capitalize text-gray-700 hover:bg-red-500 hover:text-white">
                        Delete
                    </a>
                </div>
                <div id="modelConfirm" class="fixed hidden z-50 inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full px-4 ">
                    <div class="relative top-40 mx-auto shadow-xl rounded-md bg-white max-w-md">
                        ...
                        <div class="p-6 pt-0 text-center">
                            ...
                            <h3 class="text-xl font-normal text-gray-500 mt-5 mb-6">Are you sure you want to delete this republic issuance?</h3>
                            <div class="flex  justify-center">
                                <form id="deleteForm" method="POST" action="{{ url('/republic_acts'. $republic->id) }}">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="mt-4 mb-">
          <div class="mb-3 text-xl font-bold">{{$republic->issuance->title}}</div>
          <div class="text-sm text-neutral-600 font-bold">Responsible Office: <span class="font-light">{{$republic->responsible_office}}</span></div>
          <div class="flex-1 inline-flex items-center">
            <div class="text-sm text-neutral-600 font-bold">URL link: </div>
            <a href="{{ $republic->issuance->url_link }}" class="font-bold ml-1 hover:underline" target="_blank">
             <span class=" text-blue-500 font-light">{{ $republic->issuance->url_link }}</span>
            </a>
        </div>
        </div>
          ...
          <div class="flex items-center justify-between text-slate-500">
            <div class="flex space-x-4 md:space-x-8">
              <div class="flex cursor-pointer items-center transition hover:text-slate-600">
                ...
                <span class="ml-2"> {{$republic->issuance->keyword}}</span>
              </div>
    ...
    <div class="text-start" style="color:rgb(83, 82, 82); margin-top: 15px;">
        {{-- Showing {{ $republics->firstItem() }} to {{ $republics->lastItem() }} of {{ $republics->total() }} entries --}}
    </div>
    ...
    <div class="flex justify-center items-center">
        <h1>No Republic Acts available</h1>
    </div>
    @endif
        ...
        <!-- Modal Content-->
            <div class="flex items-center justify-center p-2">
                <div class="w-full bg-white">
                    <form method="POST" action="{{route('republic.store')}}">
                        @csrf
                        <div class="mb-5">
 ...
 <script>
   document.addEventListener('DOMContentLoaded', function () {
    ...
    function openDeleteModal(id) {
        if (confirm("Are you sure you want to delete this Republic Act?")) {
