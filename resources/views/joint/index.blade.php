<x-app-layout>
    <div class="w-full max-w-[1190px] px-6 sm:px-8 md:px-16 py-2 md:py-10 rounded-xl min-h-[300px] m-2">
        ...
        <div class="flex flex-col md:flex-row gap-2">
            ...
            <div class="w-full md:w-1/2">
                <form action="{{ route('joint.index') }}" method="GET" class="mb-4" id="filterForm">
                    <div class="relative">
        ...
        <div class="w-full z-10">
            <div class="flex flex-col">
                @if(count($joints) === 0 && request()->has('search'))
                    <div class="text-gray-900 justify-center">
                        No Joint Circulars found for your search query "{{ request('search') }}".
                    </div>
                @endif

                @if(count($joints) > 0)
                    @foreach ($joints as $circular)
                        <div class='flex items-center mt-3'>
                            <div class="rounded-xl border-l-4 border-blue-700 p-5 shadow-md w-full bg-white">
                                <div class="flex w-full items-center justify-between border-b pb-3">
                                    <div class="flex items-center space-x-3">
                                        <div class="text-ml font-bold text-slate-700">Reference:
                                            <span class="font-light">{{ $circular->reference ?? 'No Reference' }}</span>
                                        </div>
                                    </div>
                                    <div class="flex items-center space-x-8">
                                        <div class="text-xs text-neutral-500">
                                            @if($circular->date)
                                                {{ \Carbon\Carbon::parse($circular->date)->format('F j, Y') }}
                                            @else
                                                N/A
                                            @endif
                                        </div>
                                ...
                                <div class="mt-4 mb-">
                                    <div class="mb-3 text-xl font-bold">{{ $circular->title ?? 'No Title' }}</div>
                                    @if(isset($circular->download_link))
                                        <div class="flex items-center">
                                            <a href="{{ asset($circular->download_link) }}" target="_blank"
                ...
                @else
                    <div class="flex justify-center items-center mt-10">
                        <h1 class="text-gray-600">No Joint Circulars available</h1>
                    </div>
                @endif
            </div>
        </div>
