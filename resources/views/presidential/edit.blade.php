    ...
    <div class="bg-white border border-4 rounded-lg shadow relative m-10">
        ...
        <div class="flex items-start justify-between p-5 border-b rounded-t">
            <h3 class="text-xl font-semibold">
                Edit Presidential Directives Details
            </h3>
            <a href="/presidential_directives" type="button" class="text-gray-400 bg-transparent hover:bg-red-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-toggle="product-modal">
               <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
            </a>
        </div>

        <div class="p-6 space-y-6">
            <form  method="POST" action="{{route('presidential.update', $presidential->id)}}">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6 sm:col-span-6">
                        ...
                        <input type="text" name="date" id="date" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required="" value="{{ old('presidential', $presidential->issuance->date) }}">
                    </div>


                    <div class="col-span-full">
                        <label for="title" class="text-sm font-medium text-gray-900 block mb-2">Title</label>
                        <textarea id="title" name="title" rows="2" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-4" >{{ old('presidential', $presidential->issuance->title) }}</textarea>
                    </div>
                    <div class="col-span-6 sm:col-span-6">
                        <label for="reference_no" class="text-sm font-medium text-gray-900 block mb-2">Reference No</label>
                        <input type="text" name="reference_no" id="reference_no" value="{{ old('presidential', $presidential->issuance->reference_no) }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required="">
                    </div>
                    <div class="col-span-full">
                        <label for="responsible_office" class="text-sm font-medium text-gray-900 block mb-2">Responsible Office</label>
                        <input type="text" name="responsible_office" id="responsible_office" value="{{ old('presidential', $presidential->responsible_office) }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required="">
                    </div>
                    <div class="col-span-full">
                        <label for="url_link" class="text-sm font-medium text-gray-900 block mb-2">Url Link</label>
                        <input type="text" name="url_link" id="url_link" value="{{ old('presidential', $presidential->issuance->url_link) }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required="">
                    ...
                    {{-- <div id="keyword-container" class="col-span-full">
                        ...
                        <input type="text" name="keyword[]" value="{{ old('presidential', $presidential->issuance->keyword) }}" class=" keyword-input shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required="">
                        @error('keyword')
                    ...
                    <div id="keyword-container" class="col-span-full">
                        ...
                        <div class="flex mb-2" id="initial-input">
                            <input type="text" name="keyword[]" placeholder="" value="{{ old('presidential', $presidential->issuance->keyword) }}"
                ...
                <div class="p-6 border-t mt-5 border-gray-200 rounded-b flex justify-end">
                    <a href="/presidential_directives" class="text-white bg-red-600 hover:bg-red-700 focus:ring-4 mr-3 focus:ring-red-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center" >Cancel</a>
